...
export const DEFAULT_REGION_DETAIL_LAYOUT: RegionDetailLayout = { tree: 40, ai: 20 };

const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));
const rounded = (value: number) => Math.round(value * 10) / 10;

function normalized(value: Partial<RegionDetailLayout>): RegionDetailLayout {
  const ai = rounded(clamp(Number(value.ai ?? DEFAULT_REGION_DETAIL_LAYOUT.ai), 15, 30));
